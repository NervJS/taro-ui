...
const renderIcon = () => {
  return <Text className='at-icon at-icon-settings' />
}
const extraStyle = {
  color: 'red',
...
describe('Card Component', () => {
  it('render initial Card', () => {
    ...
    expect(
      component.find('.at-card__header-title').length
    ).toBeGreaterThanOrEqual(1)
    expect(component.find('.at-card__content-info').first().text()).toBe(
      '测试文字'
    )
  ...
  it('render Card -- props title', () => {
    ...
    expect(component.find('.at-card__header-title').first().text()).toBe(
      '测试标题'
    )
    expect(component.html()).toMatchSnapshot()
  })
  ...
  it('render Card -- props note', () => {
    ...
    expect(component.find('.at-card__content-note').first().text()).toBe(
      'testNote'
    )
    expect(component.html()).toMatchSnapshot()
  })
  ...
  it('render Card -- props thumb', () => {
    ...
    expect(
      component.find('.at-card__header-thumb').length
    ).toBeGreaterThanOrEqual(1)
  ...
  it('render Card -- props renderIcon', () => {
    ...
    expect(
      component.find('.at-icon.at-icon-settings').length
    ).toBeGreaterThanOrEqual(1)
    expect(component.html()).toMatchSnapshot()
  })
  ...
  it('render Card -- props extra', () => {
    ...
    expect(component.find('taro-view-core')).toHaveLength(4)
    expect(component.find('taro-text-core')).toHaveLength(2)
    expect(component.find('.at-card__header-extra').first().text()).toBe(
      '额外信息'
    )
    expect(component.html()).toMatchSnapshot()
  })
  ...
  it('render Card -- props extraStyle', () => {
    const component = mount(
      <AtCard extra='额外信息' extraStyle={extraStyle}>
        测试文字
      </AtCard>
    )
    expect(component.prop('extraStyle')).toMatchObject(extraStyle)
    expect(
      component.find('.at-card__header-extra').first().prop('style')
    ).toMatchObject(extraStyle)
    expect(component.html()).toMatchSnapshot()
  })
  ...
  it('render Card -- props icon', () => {
    ...
    expect(
      component.find('.at-icon.at-icon-chevron-down.at-card__header-icon')
        .length
    ).toBeGreaterThanOrEqual(1)
    expect(component.find('.at-icon').first().prop('style')).toMatchObject(
      iconStyle
    )
    expect(component.html()).toMatchSnapshot()
  })
